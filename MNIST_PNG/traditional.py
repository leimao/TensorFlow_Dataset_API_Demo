...
def image_preprocess(images):
    ...
    for filename in images:
        ...
        #image = cv2.resize(image, (image.shape[0] * 2, image.shape[1] * 4))
        #image = cv2.resize(image, (image.shape[0] // 2, image.shape[1] // 4))
...
class CNN(object):
    ...
    def train(self, train_dataset, num_iterations):
        ...
        for i in range(num_iterations):
            ...
            images = image_preprocess(images = images)
...
def dataset_preprocessing(images, labels, shuffle = False):
...
def main():
    ...
    model = CNN(image_shapes = image_shapes, num_classes = num_classes, batch_size = batch_size, dropout = dropout, learning_rate = learning_rate)

    for i in range(num_epochs):

        train_dataset = dataset_preprocessing(images = train_images, labels = train_labels, shuffle = True)
        test_dataset = dataset_preprocessing(images = test_images, labels = test_labels, shuffle = True)
