...
class CNN(object):
    ...
    def __init__(self, dataset_output_types, dataset_output_shapes, num_classes = 10, batch_size = 16, dropout = 0.5, learning_rate = 0.001):
        ...
        self.iterator = tf.data.Iterator.from_string_handle(string_handle = self.handle, output_types = dataset_output_types, output_shapes = dataset_output_shapes)
    ...
    def train(self, train_dataset, num_iterations):
        ...
        for i in range(num_iterations):
            _, train_accuracy = self.sess.run([self.optimizer, self.accuracy], feed_dict = {self.handle: train_handle})
    ...
    def test(self, test_dataset, num_iterations):
        ...
        test_accuracies = []
        for i in range(num_iterations):
            test_accuracy = self.sess.run(self.accuracy, feed_dict = {self.handle: test_handle})
...
def read_image(filename, label):
    ...
    image = imageio.imread(filename.decode())
...
def dataset_generation(images, labels, preprocess, buffer_size = 100000, batch_size = 16, repeat = False, shuffle = False):
    ...
    dataset = dataset.map(lambda filename, label: tuple(tf.py_func(read_image, [filename, label], [tf.uint8, label.dtype])))
...
def load_mnist(dataset_dir):
    ...
    for i in range(10):
        # Training dataset
        data_dir = os.path.join(dataset_dir, 'training', str(i))
        valid_filenames = [os.path.join(data_dir, filename) for filename in os.listdir(data_dir) if os.path.isfile(os.path.join(data_dir, filename))]
        train_images += valid_filenames
        train_labels += [i] * len(valid_filenames)
        # Test dataset
        data_dir = os.path.join(dataset_dir, 'testing', str(i))
        valid_filenames = [os.path.join(data_dir, filename) for filename in os.listdir(data_dir) if os.path.isfile(os.path.join(data_dir, filename))]
        test_images += valid_filenames
        test_labels += [i] * len(valid_filenames)
...
def main():
    ...
    (train_images, train_labels), (test_images, test_labels) = load_mnist(dataset_dir = mnist_dir)

    preprocessor = Preprocessor(num_classes = num_classes)

    train_dataset = dataset_generation(images = train_images, labels = train_labels, preprocess = preprocessor.preprocess, batch_size = batch_size, repeat = True, shuffle = False)
    test_dataset = dataset_generation(images = test_images, labels = test_labels, preprocess = preprocessor.preprocess, batch_size = batch_size, repeat = False, shuffle = False)
